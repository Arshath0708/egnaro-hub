<?php
// get-vendor-shipments.php — Fetch paginated, searched shipments for a particular vendor
...
// --- Build Query Filters ---
$where = "WHERE s.vendor_id = ? AND s.status != 'payment_pending'";
$params = [$vendor_id];
$types = "i";
...
if ($search_filter !== '') {
    $where .= " AND (s.shipment_id LIKE ? OR o.order_id LIKE ? OR o.customer_name LIKE ? OR o.phone LIKE ?)";
    $like = "%" . $search_filter . "%";
    $params[] = $like;
    $params[] = $like;
    $params[] = $like;
...
// Join orders to retrieve delivery details
...
$sql = "SELECT s.id, s.shipment_id, s.order_id, s.status, s.weight_g, s.length_cm, s.width_cm, s.height_cm,
               s.awb_code, s.courier_name, s.label_url, s.manifest_url, s.created_at,
...
while ($row = $result->fetch_assoc()) {
    ...
    // Fetch tracking milestones/history
