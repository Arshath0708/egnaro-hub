<?php
// update-product.php — Update product details (vendor)
...
$vendor_id = !empty($data->vendor_id) ? intval($data->vendor_id) : 0;
...
if ($product_id > 0) {
    try {
        ...
        if ($vendor_id > 0) {
            $query = "UPDATE products SET " . implode(", ", $updates) . " WHERE id = ? AND vendor_id = ?";
            $params[] = $vendor_id;
            $types .= "i";
        }
